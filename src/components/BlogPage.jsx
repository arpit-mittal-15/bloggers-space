  ...
  return(
    <div className="max-h-[94%] h-fit w-[95%] flex flex-col justify-self-center bg-white rounded-xl my-5 overflow-scroll scroll-mb-3">
      ...
      <div>
        ...
        <div className="text-right mx-5 text-sm text-slate-600">-{blogCreator.name}</div>
        <div className="text-xl m-3">{blogData.content}</div>
      </div>
      <div className="my-5 mx-3 flex flex-row justify-between border-b-2 pb-5">
      ...
      <div id="comments" className="px-3">
        ...
        <div>
          <form className="w-full grid grid-flow-row grid-cols-[auto_100px] py-2">
            <input type="text" placeholder="Add a comment..." className="my-2 border-b-2 focus:outline-none"/>
            <button type="submit" className="bg-red-500 text-white rounded-full w-[90%] h-fit px-1 py-1 justify-self-center" onClick={handleCommentSubmit}>Post</button>
          </form>
        </div>
        <BlogComments/>
      </div>
