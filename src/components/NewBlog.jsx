...
export function NewBlog(){
  ...
  return(
    <div className="box-border mr-5 my-5">
      <form className="flex flex-col items-center gap-3" onSubmit={handleBlogSubmit} encType="multipart/form-data">
        ...
        <textarea name="content" id="content" placeholder="Write the content here.." className="w-[98%] min-h-[360px] m-auto p-2 rounded-[10px] focus:outline-none whitespace-break-spaces" value={blogData.content} onChange={(e) => {
          setBlogData((currentState) => ({...currentState, content: e.target.value}))
        }}/>
        <button type="submit" className="bg-red-500 text-white w-[200px] m-2 p-2 place-self-end rounded-full hover:scale-105 transition-all duration-300">Create</button>
      </form>
    </div>
  )
}